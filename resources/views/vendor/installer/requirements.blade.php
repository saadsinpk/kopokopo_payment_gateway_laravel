        ...
        <ul class="list">
            <li class="list__item list__title {{ $phpSupportInfo['supported'] ? 'success' : 'error' }}">
                ...
                @if($type == 'php')
                    ...
                    <span class="float-right">
                        ...
                        <i class="fas fa-fw fa-{{ $phpSupportInfo['supported'] ? 'check' : 'exclamation' }} row-icon" aria-hidden="true"></i>
                    </span>
                @endif
            </li>
            @foreach($requirements['requirements'][$type] as $extention => $enabled)
                <li class="list__item {{ $enabled ? 'success' : 'error' }}">
                    {{ $extention }}
                    <i class="fas fa-fw fa-{{ $enabled ? 'check' : 'exclamation' }} row-icon" aria-hidden="true"></i>
                </li>
            @endforeach
        </ul>
    @endforeach
